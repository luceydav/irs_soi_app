...
states <- unique(irs_app_data$state)
counties <- unique(irs_app_data$county)
cities <- unique(irs_app_data$post_office_city)
zipcodes <- unique(irs_app_data$zipcode)
agi_levels <- unique(irs_app_data$agi_level)
...
sidebar <- dashboardSidebar(
          ...
          dashboardSidebar(
            sidebarMenu(
              ...
              sidebarMenuOutput("City"),
              sidebarMenuOutput("Zipcode"),
              sidebarMenuOutput("AGI"),
              uiOutput("Search")
              )
          ))
...
server <- function(input, output, session) {
  ...
  observe({
    if(!is.null(input$State)) {
      ...
      temp <- data[state %chin% input$State]
      ...
      display <- eventReactive(input$Search,{
        ...
        if (is.null(input$County)){Counties <- unique(temp$county)}
        Cities <- input$City
        if (is.null(input$City)){Cities <- unique(temp$post_office_city)}
        Zipcodes <- input$Zipcode
        if (is.null(input$Zipcode)){Zipcodes <- unique(temp$zipcode)}
        
        ## Dplyr::filter data
        temp <-
          temp[county %chin% Counties &
                 city %chin% Cities & 
                 zipcode %chin% Zipcodes]
        return(temp)
